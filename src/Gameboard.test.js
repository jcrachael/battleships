...
it("placeShip returns error if shipType not defined", () => {
  ...
  expect(newBoard.place(shipType, coords, orientation)).toBe(
    "Error: ship type is not defined"
  );
});
...
it("placeShip returns error if orientation not defined", () => {
  ...
  let orientation;
  expect(newBoard.place(shipType, coords, orientation)).toBe(
    "Error: orientation is not defined"
  );
});
...
it("throws error when coords not from 0-9", () => {
  ...
  const coords2 = [3, -1];
  const coords3 = [-1, 11];
  expect(newBoard.place(shipType, coords1, orientation)).toBe(
    "Error: coords must be from 0-9"
  );
  expect(newBoard.place(shipType, coords2, orientation)).toBe(
    "Error: coords must be from 0-9"
  );
  expect(newBoard.place(shipType, coords3, orientation)).toBe(
    "Error: coords must be from 0-9"
  );
});
